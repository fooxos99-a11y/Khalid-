...
import { getRequestSession, isPrivilegedRole, requireAdminPermission, requireAdminPermissionsForSession, requireRoles } from "@/lib/auth/guards"
...
export async function POST(request: Request) {
	try {
		const auth = await requireAdminPermission(request, ["إدارة الاختبارات", "اختبار الطلاب"])
		if ("response" in auth) {

...
import { ensureStudentAccess, getRequestSession, isPrivilegedRole, requireAdminPermissionsForSession, requireRoles } from "@/lib/auth/guards"
...
export async function GET(request: Request) {
  try {
    ...
    if (isPrivilegedRole(session.role)) {
      const permissionAuth = await requireAdminPermissionsForSession(session, ["إدارة الاختبارات", "اختبار الطلاب"])
      if ("response" in permissionAuth) {
        return permissionAuth.response
      }
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (isPrivilegedRole(auth.session.role)) {
      const permissionAuth = await requireAdminPermissionsForSession(auth.session, "إدارة الاختبارات")
      if ("response" in permissionAuth) {
        return permissionAuth.response
      }
    }

...
export async function GET(request: Request) {
  try {
    ...
    const halaqah = String(url.searchParams.get("halaqah") || "").trim()
    const supabase = createAdminClient()

    let query = supabase
      .from("pathway_levels")
      .select("id, level_number, title, description, is_locked, points, halaqah")
      .order("level_number")
    ...
    console.error("[pathway-levels] GET:", error)

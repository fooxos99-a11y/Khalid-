...
export async function GET(request: Request) {
  try {
    ...
    const { session } = auth
    const supabase = isPrivilegedRole(session.role) ? createAdminClient() : await createClient()
    ...
    if (accountNumber) {
      ...
      const scopedSupabase = createAdminClient()
      const normalizedAccountNumber = Number.parseInt(accountNumber, 10)

      if (Number.isNaN(normalizedAccountNumber)) {
        return NextResponse.json({ error: "رقم الحساب غير صالح" }, { status: 400 })
      }

      const { data: teachers, error } = await scopedSupabase
        .from("users")
        .select("*")
    ...
    const teachersWithStudentCount = await Promise.all(
      (teachers || []).map(async (teacher) => {
        ...
        return {
          ...
          phoneNumber: teacher.phone_number || "",
          role: DEPUTY_TEACHER_ROLE_VALUES.includes(String(teacher.role || "").trim().toLowerCase()) ? "deputy_teacher" : "teacher",
        }
      })
    );

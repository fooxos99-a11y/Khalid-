...
const ALLOWED_OUTBOUND_MEDIA_MIME_TYPES = new Set(["image/jpeg", "image/png", "image/webp", "application/pdf"])
const OUTBOUND_MEDIA_MIME_BY_EXTENSION: Record<string, string> = {
  pdf: "application/pdf",
  jpg: "image/jpeg",
  jpeg: "image/jpeg",
  png: "image/png",
...
function resolveOutgoingMimeType(mimeType: string | null | undefined, fileName: string | null | undefined) {
  const normalizedMimeType = String(mimeType || "").trim().toLowerCase()
  if (ALLOWED_OUTBOUND_MEDIA_MIME_TYPES.has(normalizedMimeType)) {
    return normalizedMimeType
  }

  const extension = String(fileName || "").trim().toLowerCase().split(".").pop() || ""
  return OUTBOUND_MEDIA_MIME_BY_EXTENSION[extension] || normalizedMimeType
}
...
function normalizeOutgoingMedia(input: OutgoingMediaInput | null | undefined) {
  ...
  const mimeType = resolveOutgoingMimeType(input.mimeType, input.fileName)
  ...
  if (!ALLOWED_OUTBOUND_MEDIA_MIME_TYPES.has(mimeType)) {
    throw new Error("نوع المرفق غير مدعوم. الأنواع المسموحة: JPG و PNG و WEBP و PDF.")
  }

  if (!/^[A-Za-z0-9+/=]+$/.test(base64)) {
    throw new Error("صيغة الملف المرفوع غير صالحة.")
  }
  ...
  if (mediaSizeBytes > MAX_OUTBOUND_MEDIA_SIZE_BYTES) {
    throw new Error("حجم الملف كبير جدًا. اختر ملفًا أصغر ثم أعد المحاولة.")
  }
...
async function enqueueMessage(data: QueueMessageInput) {
  ...
  const messageType = data.media ? (data.media.mimeType === "application/pdf" ? "document" : "image") : "text"
...
async function enqueueMessagesBulk(params: {
  ...
  for (const recipient of params.recipients) {
    ...
    queuedIds.push(id)
    const messageType = resolvedMedia ? (resolvedMedia.mimeType === "application/pdf" ? "document" : "image") : "text"
    queueRows.push({
      id,
  ...
  if (queueRows.length > 0) {
    await insertQueueRows(supabase, queueRows, { includesMedia: queueRows.some((row) => row.message_type !== "text") })
...
export async function GET(request: Request) {
  try {
    ...
    if (ids.length > 0) {
      ...
      const resolvedStatuses = (statuses || []) as MessageStatusRow[]
      const summary = resolvedStatuses.reduce<MessageStatusSummary>((result, status) => {
        result.tracked += 1
      ...
      return NextResponse.json({ statuses: resolvedStatuses, summary })
    }

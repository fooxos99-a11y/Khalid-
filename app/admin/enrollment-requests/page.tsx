...
export default function EnrollmentRequestsPage() {
  return <EnrollmentRequestsContent displayMode="page" />;
}

export function EnrollmentRequestsContent({
  displayMode = "page",
  onInlineActionsChange,
}: {
  displayMode?: "page" | "inline"
  onInlineActionsChange?: (actions: {
    toggleEnrollmentStatus: () => void
    openTemplates: () => void
    copyEnrollmentLink: () => void
  ...
  }, [onInlineActionsChange, isEnrollmentOpen, isStatusLoading, copiedLink])
  ...
  return (
    <div className={`${displayMode === "inline" ? "bg-transparent" : "min-h-screen bg-[#f6f7f9]"} flex flex-col font-cairo`} dir="rtl">
      {displayMode === "inline" ? null : <Header />}

      <main className={`flex-grow ${displayMode === "inline" ? "px-0 py-0" : ""}`}>
        <div className={`mx-auto flex max-w-7xl flex-col gap-6 ${displayMode === "inline" ? "px-0 py-0" : "px-4 py-8"}`}>
          {displayMode === "inline" ? null : (
            <div className="flex flex-wrap items-center justify-end gap-3 px-1">
              <button
                ...
                className={`inline-flex h-12 items-center gap-2 rounded-full border px-5 text-sm font-bold transition-all ${
                  isEnrollmentOpen
              ...
              >
                {isStatusLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : isEnrollmentOpen ? <Lock className="h-4 w-4" /> : <Unlock className="h-4 w-4" />}
                <span>{isEnrollmentOpen ? "إقفال التسجيل" : "فتح التسجيل"}</span>
              </button>
              <button
                onClick={copyEnrollmentLink}
                className="inline-flex h-12 items-center gap-2 rounded-full bg-[#3453a7] px-5 text-sm font-bold text-white shadow-[0_10px_30px_rgba(52,83,167,0.16)] transition-all hover:bg-[#24428f] focus-visible:bg-[#24428f]"
              >

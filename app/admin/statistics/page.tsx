...
export default function StatisticsPage() {
  return <StatisticsContent displayMode="page" />;
}
...
export function StatisticsContent({
  ...
  const [selectedCircleReport, setSelectedCircleReport] = useState<string | null>(null);
    ...
    <div className="space-y-8">
      <section className="p-2 md:p-0">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          ...
          <div className="w-full max-w-xs text-right md:flex-shrink-0">
            <div className="relative">
              ...
              <Select value={dateFilter} onValueChange={(value) => handleDateFilterChange(value as DateFilter)}>
                <SelectTrigger className="h-12 w-full flex-row-reverse rounded-xl border border-[#d9e2ec] bg-white px-4 text-right text-sm font-bold text-[#1a2332] shadow-sm transition focus:ring-[#3453a7]/20 data-[placeholder]:text-[#64748b] [&_[data-slot=select-value]]:justify-end [&_[data-slot=select-value]]:text-right">
                  <SelectValue className="text-right" />
                </SelectTrigger>
                <SelectContent dir="rtl" className="rounded-xl border border-[#dfe7f5] bg-white p-2 shadow-[0_18px_50px_rgba(15,23,42,0.12)]">
                  {Object.entries(FILTER_LABELS).map(([value, label]) => (
                    <SelectItem key={value} value={value} className="cursor-pointer rounded-lg text-right font-bold text-[#1f2a3d] focus:bg-[#f5f8fd] focus:text-[#20335f]">
                      {label}
                    </SelectItem>
                  ))}
                </SelectContent>
      ...
      {loading ? (
        ...
        <>
          ...
          <section className="space-y-3">
            ...
            {selectedCircleReport ? (
              <div className="rounded-[28px] border border-[#e2eaf7] bg-white p-4 shadow-[0_18px_40px_rgba(15,23,42,0.05)] sm:p-6">
                <CircleWeeklyReports
                  circleName={selectedCircleReport}
                  displayMode="embedded"
                  showBackButton
                  backLabel="العودة إلى بطاقات الحلقات"
                  onBack={() => setSelectedCircleReport(null)}
                />
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {allCircles.map((circle) => {
                  const circleName = circle.name || TEXT.unknownCircle;

                  return (
                    <button
                      key={circle.id}
                      type="button"
                      onClick={() => setSelectedCircleReport(circleName)}
                    ...
                    >
                      ...
                      <div className="relative">
                        ...
                        <div className="line-clamp-2 text-base font-black leading-8 text-[#1a2332] sm:text-[1.15rem]">{circleName}</div>
                      </div>
                    </button>
                  );
                })}
  ...
  if (displayMode === "inline") {
    return <div className="bg-[#fafaf9] px-1 py-1 font-cairo" dir="rtl">{content}</div>;
  }
